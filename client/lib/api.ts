...
export default class Api {
  ...
  async login(result: UserLogin): Promise<undefined | string> {
    ...
    if (this.callback && !this.callbacked.value)
      this.callback()
  ...
  private async setUser(): Promise<void> {
    try {
      const result = await $fetch<api.HarmonyResponse & { data: models.User }>(
        '/me',
        this.fetchOptions({ params: undefined, method: 'GET' }),
      )
  ...
  public async index<Results>(
    endpoint: string,
    params?: SearchParameters,
  ): Promise<api.HarmonyResponse & { data: Results }> {
    try {
      if (params) {
        endpoint += '?'
        for (const key in params) {
          if (Array.isArray(params[key]))
            for (const item of params[key]) endpoint += `${key}[]=${item}&`
          else endpoint += `${key}=${params[key]}&`
        }
      }

      return await $fetch<api.HarmonyResponse & { data: Results }>(
        endpoint,
        this.fetchOptions({ params: undefined, method: 'GET' }),
      )
    }
  ...
  public async get<Result>(
    endpoint: string,
    params?: SearchParameters,
  ): Promise<api.HarmonyResponse & { data: Result }> {
    try {
      return await $fetch<api.HarmonyResponse & { data: Result }>(
        endpoint,
        this.fetchOptions({ params, method: 'GET' }),
      )
    }
    catch (error) {
      await this.toastError(error)
  ...
  public async update(
    endpoint: string,
    params?: SearchParameters,
  ): Promise<api.HarmonyResponse> {
    try {
      return (
        await $fetch<api.HarmonyResponse & { data: api.HarmonyResponse }>(
          endpoint,
          this.fetchOptions({ params, method: 'PUT' }),
        )
      ).data
    }
    catch (error) {
  ...
  public async store<Result>(
    endpoint: string,
    params?: SearchParameters,
    cb?: (e: FetchError) => unknown,
  ): Promise<api.HarmonyResponse & { data: Result }> {
    try {
      return (
        await $fetch<
          api.HarmonyResponse & { data: api.HarmonyResponse & { data: Result } }
        >(endpoint, this.fetchOptions({ params, method: 'POST' }))
      ).data
    }
    catch (error) {
      if (cb)
  ...
  public async delete(
    ...
    try {
      return (
        await $fetch<api.HarmonyResponse & { data: api.HarmonyResponse }>(
          endpoint,
          this.fetchOptions({ params, method: 'DELETE' }),
        )
      ).data
    }
    catch (error) {
  ...
  public async attempt(token: string | string[]): Promise<UserLogin> {
    try {
      return (
        await $fetch<api.HarmonyResponse & { data: UserLogin }>(
          '/login',
          this.fetchOptions({ params: { token }, method: 'POST' }),
        )
      ).data
    }
    catch (error) {
  ...
  public async logout(router: Router): Promise<void> {
    const response = await $fetch<api.HarmonyResponse>(
      '/logout',
      this.fetchOptions({ params: undefined, method: 'GET' }),
    )
